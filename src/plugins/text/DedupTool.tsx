...
export default function DedupTool() {
  ...
  const inputCount = mode === "line" ? input.split("\n").length : mode === "word" ? input.split(/\s+/).filter(Boolean).length : input.split("\n").filter((l) => l.trim()).length;
  const outputCount = mode === "line" ? output.split("\n").length : mode === "word" ? output.split(/\s+/).filter(Boolean).length : output.split("\n").filter((l) => l.trim()).length;
  ...
  return (
    <Card title="文本去重" bordered={false}>
      ...
      <Space style={{ margin: "12px 0" }}>
        <Button type="primary" onClick={dedup}>去重</Button>
      ...
      {output && (
        <Row gutter={16} style={{ marginBottom: 12 }}>
          <Col span={6}><Statistic title="去重前" value={inputCount} suffix={mode === "line" ? "行" : "词"} /></Col>
          <Col span={6}><Statistic title="去重后" value={outputCount} suffix={mode === "line" ? "行" : "词"} /></Col>
          <Col span={6}><Statistic title="删除" value={inputCount - outputCount} suffix={mode === "line" ? "行" : "词"} /></Col>
        </Row>
      )}
