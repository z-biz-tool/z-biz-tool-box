...
import { Input, Button, Space, Card, message, Row, Col, Statistic, Select, DatePicker, Radio, Tag } from "antd";
...
export default function TimestampTool() {
  ...
  const convertTs = () => {
    if (!timestamp) { message.warning("请输入时间戳"); return; }
    const ts = parseInt(timestamp, 10);
    if (isNaN(ts)) { message.error("无效的时间戳"); return; }
  ...
  return (
    <Card title="时间戳转换" bordered={false}>
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        <Select value={timezone} onChange={setTimezone} options={TIMEZONES} style={{ width: 240 }} />
      </Space>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}>
          <Card size="small">
            <Button size="small" onClick={getNow} style={{ marginTop: 4 }}>获取时间戳</Button>
      ...
      {mode === "ts2date" ? (
        <>
          <Space style={{ marginBottom: 12 }}>
            ...
            />
            <Button type="primary" onClick={convertTs}>转换</Button>
            <Button onClick={() => setTimestamp(String(now))}>填入当前</Button>
          </Space>
          {dateStr && (
            <Card size="small" type="inner">
              <Row gutter={16}>
                <Col span={6}><Statistic title="本地时间" value={dateStr.format("YYYY-MM-DD HH:mm:ss")} /></Col>
                <Col span={6}><Statistic title={`${timezone}`} value={formatInTz(dateStr)} /></Col>
                <Col span={6}><Statistic title="UTC" value={dateStr.utc().format("YYYY-MM-DD HH:mm:ss")} /></Col>
                <Col span={6}><Statistic title="ISO 8601" value={dateStr.toISOString()} valueStyle={{ fontSize: 12 }} /></Col>
              </Row>
              <div style={{ marginTop: 8 }}>
                <Tag color="blue">星期{["日", "一", "二", "三", "四", "五", "六"][dateStr.day()]}</Tag>
        ...
        <>
          <Space style={{ marginBottom: 12 }}>
            <DatePicker
              ...
              style={{ width: 250 }}
            />
            <Button type="primary" onClick={convertDate}>转换</Button>
            <Button onClick={() => setDateStr(dayjs())}>填入当前</Button>
          </Space>
          {timestamp && (
            <Card size="small" type="inner">
              <Row gutter={16}>
                <Col span={12}><Statistic title="时间戳(秒)" value={timestamp} /></Col>
                <Col span={12}><Statistic title="时间戳(毫秒)" value={parseInt(timestamp, 10) * 1000} /></Col>
              </Row>
            </Card>
          )}
        </>
