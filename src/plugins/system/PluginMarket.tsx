...
import { Card, Row, Col, Tag, Typography, Input, Select, Button, Space, Empty, message, Statistic, Badge } from "antd";
...
// 本地工具元数据（前端实现的工具）
const LOCAL_TOOLS: { id: string; name: string; description: string; category: string; icon: string }[] = [
  // 编码
  { id: "url", name: "URL 编解码", description: "URL 编码/解码", category: "编码", icon: "link" },
  { id: "html-entity", name: "HTML 实体", description: "HTML 实体编解码", category: "编码", icon: "code" },
  ...
  // 文本
  { id: "json-format", name: "JSON 工具", description: "格式化/转换/提取", category: "文本", icon: "code" },
  { id: "diff", name: "文本对比", description: "文本 Diff", category: "文本", icon: "diff" },
  { id: "case", name: "大小写转换", description: "大小写/驼峰/下划线", category: "文本", icon: "font" },
  ...
  { id: "wordcount", name: "字数统计", description: "字符/单词/行", category: "文本", icon: "stats" },
  { id: "reverse", name: "文本翻转", description: "字符/单词/行翻转", category: "文本", icon: "swap" },
  ...
  // 转换
  { id: "color", name: "颜色转换", description: "HEX/RGB/HSL 互转", category: "转换", icon: "color" },
  { id: "numberbase", name: "进制转换", description: "二/八/十/十六进制", category: "转换", icon: "number" },
  { id: "unit", name: "单位换算", description: "长度/重量/温度等", category: "转换", icon: "ruler" },
  { id: "exchange", name: "汇率换算", description: "货币汇率转换", category: "转换", icon: "money" },
  { id: "cron", name: "Cron 解析", description: "Cron 表达式中文描述", category: "转换", icon: "clock" },
...
export default function PluginMarket() {
  ...
  useEffect(() => {
    invoke<PluginInfo[]>("list_plugins").then((result) => {
      setPlugins(result);
    }).catch(() => {
      setPlugins([]);
    });
    ...
    <Card title="插件市场" bordered={false}>
      ...
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        {CATEGORIES.filter((c) => c !== "全部").map((cat) => (
          <Badge key={cat} count={allTools.filter((t) => t.category === cat).length} offset={[0, 0]}>
            <Tag
          ...
          {filtered.map((tool) => (
            <Col xs={24} sm={12} md={8} lg={6} key={tool.id}>
              <Card
                ...
                actions={[
                  <Button size="small" type="link" onClick={() => copy(tool.id)}>复制ID</Button>,
                ]}
              >
                ...
                <div style={{ marginTop: 8 }}>
                  <Tag color="blue" style={{ fontFamily: "monospace" }}>{tool.id}</Tag>
